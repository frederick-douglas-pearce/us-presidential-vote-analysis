...
**The non-obvious half, and the reason this is a decision rather than a chore:** the cache-busted reachability check *cannot* be reused for the edge assertion. The Worker keys `caches.default` on the request, query string included, so a per-run url is a distinct key — always a MISS, always fetched from the origin, always carrying the new ETag. It is structurally blind to a failed purge. Only the canonical url, the one real users hit and the only key holding a pre-purge entry, tests whether the edge cut over. That matters more than it looks: the Worker's edge hold is `s-maxage` 30 days (D035, which replaced D034 §4's Cache Rule), so a purge that silently no-ops serves stale data for a month while every status-code assertion stays green. **Do not "tidy" the cache-buster onto this check** — it would disable it without changing a single line of its logic.

**What this check does not prove.** `caches.default` is **colo-local**, and one runner request samples one colo. So it establishes that *the colo the runner hit* cut over, not that every colo did. It catches a total purge failure — the likely mode, and the one worth a gate — but a purge that evicts the runner's colo and leaves another stale passes green while users in that region read month-stale data. This is not cheaply fixable from a single runner at deploy time. The compensation is #194's canary, which asserts edge/origin *agreement* oracle-free on a schedule and so samples many colos over time. The residual seam both together still leave: one non-runner colo serving stale between a silently-failed purge and the next canary run. Accepted — the data refreshes about every four years and the API is public and read-only — but recorded so it is a known seam rather than a surprise.
...
(a)'s `Total` anchor carries the same shape of risk, and the mutant it defeats is **narrower than
this entry first claimed**. On both real sheets a caption row — `(leading dots indicate sub-parts)` —
sits between the header and `Total`, and its cells are empty, so the wide substitution "the first row
after the header" reads blanks, returns NULL populations, and the accept-side assertion on Alaska
1950 kills it unaided. What the published bytes **cannot** distinguish is "the first row after the
header *that carries a parseable number*", which lands on `Total` on both sheets precisely because
that caption carries none. That is the mutant the synthetic sheet — race rows *first* — exists to
kill, and it is pinned there.

**Corrected before merge (#234 review).** This paragraph first said the wide substitution was
byte-identical across every published byte and unkillable by any test reading the fixture or the
corpus. Neither is true, and the plan-time architect pass recorded the same claim, so the plan gate's
own record inherited it. This is #182's surviving-mutant lesson applied before the fact rather than after: there,
...
## D062: D060's coverage-exception paragraph and its Action-required section are superseded

**Date:** 2026-09-13
**Issue:** #234 (E10) · **Supersedes:** D060 §(h) and D060's "Action required" section ·
**Builds on:** D061

**Context.**

This log is append-only, and the working rule is that only its **most recent** entry may be edited in
place. D060 is no longer that entry — D061 sits after it — so two passages in D060 that #234 has made
**false** rather than merely dated cannot be corrected where they stand. They are passages a reader
arrives at directly: `conform.py`'s doc-comment and `CLAUDE.md` both cite D060 for the
coverage-exception story.

**Decision.**

**Read D060 §(h) and D060's "Action required" section with this entry attached.**

- §(h) records three entries in two kinds, and says 1960 Alaska and 1960 Hawaii **are**
  `present_but_unparsed` because "the figures exist ... in a transposed second table the parser does
  not read". As of #234/D061 the parser **does** read it. The catalog holds **one** entry —
  `(1848, Texas)`, `absent_from_source` — and `KIND_PRESENT_BUT_UNPARSED` stays defined with no
  member, deliberately (D061(c)).
- "Action required" asks for a follow-up issue covering the transposed second table, and states that
  "until it lands, 1960 Alaska and 1960 Hawaii persons-per-electoral-vote are NULL". That follow-up
  **was** #234 and it has landed: both cells now carry 128,643 and 499,794. **Its second clause
  remains open** — whether #184 takes the apportionment denominator at 1924/1928 or a
  freshest-enumeration second field (D060(c)) — and this entry does not touch it.

Nothing in D060 is rewritten. The superseded text stands in the record as what the project believed
when it shipped.

**Rationale.**

Supersession-in-place is this log's own idiom for a correction: D060 §(i) corrected D059 point 3
without editing it — *"the earlier statement stands in the record and is superseded here"* — and D035
revised D034's mechanism the same way. The alternative, editing D060, would breach the append-only
guard and erase the history the guard exists to keep.

A separate entry rather than a sentence inside D061 is the maintainer's call at #234's scope gate, and
it is the one that matches the rule above: D061 is itself an entry, not a place to amend an earlier
one, and a reader who reaches D060 from `CLAUDE.md` needs the pointer at log level rather than buried
in a neighbour's prose.

**Action required.** None. #234's acceptance criterion 5 — *"`docs/corrections.md`'s
coverage-exception table **and D060** are updated to match; the two kinds stay distinct"* — is
discharged by this entry under the supersession reading, which the maintainer approved at the #234
scope gate; the criterion's wording was amended on the issue to say so.
...
**Date:** 2026-09-21
**Issue:** #253 (E10) · **Supersedes:** D060 §(f)'s "1824–1860" span ·
...
is only ever a **vintage** selector — `census/conform.py:456` picks which census year's population
to merge, `census/reconcile.py:26,306,340,372` picks which apportionment's seats to read — and never
...
it `at_election` (`conform.py:198-214`, `:488-580`, and `:579` writes the label).
...
- **The blast radius is aggregates only.** Virginia holds **zero** electoral votes in 1864 and 1868,
  ...
  over an election year. `conform.py:190-197` and `docs/corrections.md` already say so.
...
1836, 1840, 1844) × Virginia` carry `at_election` — `conform.py:462-465` maps `as_enumerated` to
`at_election`, and `census/transform.py:112` makes every 1790–1860 Virginia census row
...
`apportionment.py:107-125` already defaults that way. A frozen list under-corrects **silently** if
D010 widens the spine below 1824, where the 1800 (5,949) and 1810 (8,552) Alexandria cells become
live.

**(g) Alexandria is SIX elections: 1824, 1828, 1832, 1836, 1840, 1844.** `apportionment.py:88-104`
...
"seven" is the apportionment-basis count and is superseded by (a). The rank flip §1 reports for
1832/1836/1840 is untouched — all three are inside the six.

**(h) The mechanism, ruled here because the count alone is not implementable.** #251 places
`ALEXANDRIA_RETROCESSION` at **census** grain, in `census/transform.py`. A census-grain fix to the
1840 row reaches **every election that census governs** — 1844 *and* 1848 — silently delivering
seven. And the naive election-grain repair raises: `assert_no_interpolated_population`
(`conform.py:681-726`) admits only the census table's own figure or a declared
`BOUNDARY_SUCCESSIONS` literal.

**That guard cannot express the answer as it stands, and this is the load-bearing detail.** It keys
`allowed_restatements` on **`(census_year, predecessor)`** (`conform.py:703-707`) — a *census*-grain
key. Both 1844 and 1848 resolve to `(1840, "Virginia")`, so a `BOUNDARY_SUCCESSIONS`-shaped entry
**cannot distinguish them at all**: whichever value it pins, the other election is refused. So:

- **Census grain states borders-at-census.** `ALEXANDRIA_RETROCESSION` stays in `transform.py`, per
  #251 and #208 recommendation 1 — which is also what finally makes `basis = as_enumerated` *true*
  for those rows.
- **Election grain reverses it where the two diverge** — one entry covering `(1848, Virginia)`, in
  the `BOUNDARY_SUCCESSIONS` idiom, restoring the Alexandria-inclusive figure because that **is**
  the 1848 footprint.
- **The allowed-restatement guard gains an election-grain key**, so a restatement can be pinned per
  `(election_year, state)` and not only per `(census_year, state)`.
- **One test**, over that entry.

All four are **#251's to implement.** This entry rules the shape so #251 inherits something
buildable rather than discovering the refusal.

**(i) Ohio DOES carry a Toledo Strip residual for 1840, and it stays IMMATERIAL.** Under (a) Ohio
held the strip at the 1840 election while the file leaves the strip's people in Michigan's column,
so Ohio's figure is short by exactly what Michigan's is long — an **equal-and-opposite residual on
both sides**, which is what #208 §6.5 declines to clear Ohio of. It remains immaterial because
condition 3 of §208 §3's threshold fails: **no source gives the strip's population**, so there is
nothing to correct with. This entry settles the *criterion*; it cannot settle the *magnitude*, and
§6.5's ruling rests on unquantifiability alone — an ATTRIBUTED finding, named here rather than
laundered into a verified one. The symmetric half matters and is easy to lose: the cession took
...
(`:26, 306, 340, 372`) and compares published seats against the recorded allotment. That is an
...
**(k) No figure moves — but three shipped statements become false and are repaired here.** They are
the same sentence in three places, all reading that for the elections **1824–1860** Virginia's
restated figure is borders-at-election. False for 1824–1844, whose figures include Alexandria:

1. `src/usvote/census/conform.py:120-121` — the `BOUNDARY_AT_ELECTION` doc-comment.
2. `docs/corrections.md`, "Census population boundary basis (#181)" — the identical sentence, in a
   **published** document.
3. **D060 §(f)** — superseded here, never edited, on the D062 precedent.

**The correct span is 1848–1860, with 1824–1844 pending #251.** Nothing in D060 is rewritten; the
superseded text stands in the record as what the project believed when it shipped.
...
Ruling the mechanism alongside the count (h) is a deliberate widening of what a "decision" issue
delivers, taken at the architect gate. The narrow reading — rule the count, let #251 work out how —
was the plan under review, and it fails: the orchestrator's own re-check of the guard found that the
six-election answer has **no legal expression** under the current key shape. A decision that cannot
be implemented is not a decision, so the shape is part of the ruling.
...
mint one — a third enum value would put a transient state into a CHECK-constrained column and reach
the public API through `dwh.election_per_capita`, for a condition that #251 removes.

**Action required.**

**#251 inherits (f), (g) and (h)**: the span as a rule, six elections, and the four-part mechanism.
Its body said "seven" and carried a stale Toledo line; both are amended, and #208 stays open until
#251 lands.

**#245** should restate its sequencing with the corrected count. Note also D064's scope record: a
**new** snapshot table sits outside the content hash, so folding per-capita rows into that hash is
#245's own acceptance criterion and is not made redundant by anything here.
