...
[`src/usvote/join.py`](../src/usvote/join.py) (D026): a **FULL OUTER _participant_ view**
(`ec_pv_preferred` for analysis, `ec_pv_redistributable` for the API) at the canonical
`(year, state, candidate)` grain — full-outer, not EC-left, so a candidate who *lost* a
state's electoral votes keeps their per-state row (electoral votes `0`, popular votes
intact), which is exactly the "lost the EC, won the PV" data the thesis explores. That
module is also where **this page's promised reciprocal guard runs**: every reconciled PV
`candidate`/`state` value must be present in the EC dims, enforced as a view-creation
precondition — an unmatched value fails loud there, it does not vanish in an inner join.
